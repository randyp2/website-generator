-- Reconciles claims previously promoted by user-controlled repository metadata alone.
...
WHERE claim.claim_type = 'skill'
  ...
  AND EXISTS (
      ...
      WHERE discovery_link.claim_id = claim.id
        ...
        AND lower(btrim(discovery_link.link_type)) IN (
            'topic_match',
            'name_match',
            'description_match'
        )
  ...
  AND NOT EXISTS (
      ...
      WHERE scoring_link.claim_id = claim.id
        ...
        AND lower(btrim(scoring_link.link_type)) NOT IN (
            'topic_match',
            'name_match',
            'description_match'
        )
...
COMMENT ON COLUMN public.claim_evidence_links.link_type IS
    'Evidence relationship type. Topic, name, and description matches are discovery provenance excluded from verification progress.';
