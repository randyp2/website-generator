-- Reconciles claims previously promoted by description-only repository links.
...
WHERE claim.claim_type = 'skill'
  ...
  AND EXISTS (
      ...
      WHERE discovery_link.claim_id = claim.id
        ...
        AND lower(btrim(discovery_link.link_type)) = 'description_match'
  )
  AND NOT EXISTS (
      SELECT 1
      ...
      WHERE scoring_link.claim_id = claim.id
        ...
        AND lower(btrim(scoring_link.link_type)) <> 'description_match'
        AND scoring_link.link_confidence >= 0.50
  );

COMMENT ON COLUMN public.claim_evidence_links.link_type IS
    'Evidence relationship type. description_match is discovery provenance and is excluded from verification progress.';
