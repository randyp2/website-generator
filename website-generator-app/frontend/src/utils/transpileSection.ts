...
/**
 * Shared dependency scope injected into every transpiled section.
 * Matches the globals exposed by the backend PortfolioHtmlExportService
 * so that AI-generated sections render identically in both paths.
 */
const SECTION_SCOPE: Record<string, unknown> = {
    // React core
    React,
    useState,
    useEffect,
    ...
    // Common animation constants (mirrored from backend)
    once: true,
    y: 0,
    x: 0,
    opacity: 1,
    ...
    // Lucide icons — full set from PortfolioHtmlExportService
    Mail,
    Phone,
    MapPin,
    Globe,
