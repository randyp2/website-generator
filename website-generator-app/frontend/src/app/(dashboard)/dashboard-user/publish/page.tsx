...
const PublishPage = () => {
  ...
  const { applyPublished, unpublish, updateDescription } = usePublishMutations({
    setPortfolios,
  })
  ...
  const ownerName =
    user?.username?.trim() || user?.email?.trim() || "You"
  ...
  const handleSaveDescription = useCallback(
    async (nextDescription: string): Promise<void> => {
      ...
      await updateDescription(String(featuredPortfolio.id), nextDescription)
    ...
    <div className="space-y-6 px-4 py-8 md:px-6">
      ...
      <div>
        <PublishHero
          ...
          copied={Boolean(
            featuredPortfolio?.slug && copiedSlug === featuredPortfolio.slug,
          )}
