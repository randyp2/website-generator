...
interface PublishWizardModalProps {
  ...
  onClose: () => void
  onPublished: (portfolioId: string, slug: string, description: string) => void
}
...
export const PublishWizardModal = ({
  ...
  const [publishState, setPublishState] = useState<PublishActionState>("idle")
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      onPublished(String(selectedPortfolio.id), data.slug, descriptionInput)
      setTimeout(() => {
        onClose()
      }, 1800)
    } catch (err: unknown) {
  ...
  const handleCopyUrl = async () => {
    ...
    await navigator.clipboard.writeText(`${origin}/portfolio/${publishedSlug}`)
    setCopied(true)
