 ...
 * 
 * 1. DUAL ZONE LAYOUT
 *    - Resume and media clearly separated
 *    - Equal visual weight (no hierarchy)
 *    - Side-by-side on desktop, stacks on mobile
 *    - Each zone is self-contained
 * 
 * 2. DRAG-DROP UX
 *    - Dashed border suggests "drop here"
 *    - Hover changes border color (feedback)
 *    - Drag-over state: blue background glow
 *    - Upload icon scales up on drag-over
 *    - Alternative "browse" link for familiarity
 * 
 * 3. UPLOAD PROGRESS
 *    - Full overlay prevents interaction during upload
 *    - Spinning loader shows system working
 *    - Progress bar with percentage (transparency)
 *    - Smooth 0→100% animation
 * 
 * 4. FILE MANAGEMENT
 *    - List appears after first upload
 *    - File type icons aid recognition
 *    - Hover reveals preview/delete actions
 *    - "Download All" for batch operations
 *    - Delete confirmation via button press
 * 
 * 5. PREVIEW MODAL
 *    - Click outside to close (intuitive)
 *    - Large preview area for images
 *    - PDF shows download option (browser limits)
 ...
 *    - Could add size validation (future)
 *    - Visual feedback for accepted/rejected
 * 
 * 7. SUCCESS FEEDBACK
 *    - Files slide in from left on complete
 *    - Progress bar completes smoothly
 *    - List expands with height animation
