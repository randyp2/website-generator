...
const PreviewContainer: React.FC<PreviewContainerProps> = ({
    ...
    useEffect(() => {
        if (isLoading) {
            ...
            const shuffledDurations = phaseDurations.sort(
                () => Math.random() - 0.5,
            );
    ...
    const handleEmailSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const { fileName, fileContent } =
                await prepareAnonymousPreview(generatedHTML);
                                                ...
                                                .map((char, index) => {
                                                    ...
                                                    if (isCenter) {
                                                        scale =
                                                            1 +
                                                            centerIntensity *
                                                                0.2; // ⚡ LINE 371: Letter peak scale (current: 0.2 = 20% larger)
                                                    } else if (isInWave) {
                                                        scale =
                                                            1 -
                                                            (1 -
                                                                waveIntensity) *
                                                                0.1; // ⚡ LINE 373: Letter valley scale (current: 0.1 = 10% smaller)
                                                    }
                                        ...
                                        {Array.from({ length: totalBars }).map(
                                            (_, index) => {
                                                ...
                                                if (isCenter) {
                                                    scaleY *=
                                                        1 +
                                                        centerIntensity * 0.3; // ⚡ Reduced peak scaling
                                                } else if (isInWave) {
                                                    scaleY *=
                                                        1 -
                                                        (1 - waveIntensity) *
                                                            0.05; // ⚡ Reduced valley scaling
                                                }
                            ...
                            >
                                <div className="text-center max-w-md mx-auto px-6">
                                    ...
                                    <p className="text-slate-600 leading-relaxed mb-6">
                                        ...
                                        "Generate Portfolio" to see your
                                        professional portfolio come to life in
                                        real-time.
                                    </p>
