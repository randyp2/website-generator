...
export async function POST(
    ...
    if (!portfolioId || !userPrompt) {
        return NextResponse.json(
            { error: "portfolioId and userPrompt are required" },
            { status: 400 },
        );
    }
    ...
    const rateLimitKey: string = `clarify_portfolio:user:${user.id}`;
    ...
    try {
        ...
        const res = await fetch(`${backendUrl}/api/v1/portfolio/refine/clarify`, {
            method: "POST",
            headers: {
        ...
        console.error("Clarify request error:", err);
        return NextResponse.json(
